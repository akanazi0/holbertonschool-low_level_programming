...
session_t *session_create(const char *id, unsigned int uid, const unsigned char *data, size_t data_len)
{
    ...
    /* 1. حجز ذاكرة خاصة للـ id ونسخه لضمان ملكية الذاكرة */
    ...
    /* 2. حجز الـ data وتفريغ الذاكرة السابقة في حال الفشل */
    if (data_len > 0) {
...
int session_set_data(session_t *s, const unsigned char *data, size_t data_len)
{
    ...
    /* 3. التعامل الآمن مع realloc دون إتلاف المؤشر القديم عند الفشل */
    tmp = (unsigned char *)realloc(s->data, data_len);
    if (!tmp) {
        return 0; /* نحتفظ بالبيانات القديمة كما هي دون تسريب */
    }
